...
const timerBar = document.getElementById("circle");
...
function renderQuestion() {
  ...
  stopAnimation(); // Assicura che non resti l'animazione dei timer precedenti

  const currentQuestion = questions[questionNumber];
  counter.innerHTML = `Question <span class="current">${
    questionNumber + 1
  ...
  options.forEach((answerText) => {
    ...
    input.addEventListener("click", () => {
      ...
      stopAnimation(); // Ferma l'animazione del timer al click sulla risposta
    });
  ...
  timerBar = setInterval(startAnimation); // Quando parte la domanda parte anche l'animazione
}
...
function countdown() {
  if (timeLeft <= 0) {
    ...
    startAnimation(); // Fa ricominciare l'animazione quando il timer arriva a zero
  } else {
    element.innerHTML = timeLeft;
    timeLeft--;
  }
...
// Funzione per gestire la partenza dell'animazione per il conto alla rovescia
function startAnimation() {
  timerBar.classList.add("circle-animation"); // Aggiunge la classe con le istruzioni dell'animazione
}

// Funzione per fermare l'animazione quando una risposta viene cliccata
function stopAnimation() {
  timerBar.classList.remove("circle-animation"); // Rimuove la classe per non far partire l'animazione
}
...
function showResults() {
  ...
  document.getElementById(
    "score"
  ).innerText = `${finalScore}/${questions.length} questions`;
}
